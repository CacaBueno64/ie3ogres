...
BOOL PackHeaderGetOffsetAndSize(void *pkh, u32 code, s32 *outOffset, size_t *outSize)
{
    ...
    while (min < max)
    {
        ...
        if (code > curCode) {
            ...
            switch (type) {
                case PACK_TYPE_COS:
                    if (outOffset) *outOffset = static_cast<PackEntry_COS *>(entry)->offset;
                    if (outSize) *outSize = static_cast<PackEntry_COS *>(entry)->size;
                    break;
                case PACK_TYPE_CO:
                    if (outOffset) *outOffset = static_cast<PackEntry_CO *>(entry)->offset;
                    if (outSize) *outSize = header->entrySize;
                    break;
                case PACK_TYPE_CS:
                    if (outOffset) *outOffset = cur * header->entrySize;
                    if (outSize) *outSize = static_cast<PackEntry_CS *>(entry)->size;
                    break;
                case PACK_TYPE_C:
                    if (outOffset) *outOffset = cur * header->entrySize;
                    if (outSize) *outSize = header->entrySize;
...
void GetBodyModelOffsetAndSize(void *pkh, Unit* unit, int wearNo, int wearType, s32 *outOffset, size_t *outSize)
{
    ...
    if ((unit->base.gender == GENDER_FEMALE) &&
        ((wearType == 0) || (wearType == 2)) &&
    ...
    PackHeaderGetOffsetAndSize(pkh, (wearSet->unk8 * 10000) + (unit->base.bodyType * 100) + (bodyNo), outOffset, outSize);
}
...
void GetShoesModelOffsetAndSize(void *pkh, Unit *unit, int shoesNo, s32 *outOffset, size_t *outSize)
{
    ...
    PackHeaderGetOffsetAndSize(pkh, (shoesInfo->unk0 * 10000) + (unit->base.bodyType * 100) + (shoesInfo->unk1), outOffset, outSize);
}
...
BOOL SFPGetOffsetAndSize(char *filename, char *file, s32 *outOffset, size_t *outSize)
{
    ...
    for (idx = 0; idx < entryCount; idx++) {
        if (strcmp(file + entries[idx].stringOffset, filename) == 0) {
            break;
        }
    }
