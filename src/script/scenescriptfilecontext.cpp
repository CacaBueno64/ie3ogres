#include "scenescriptmanager.hpp"
...
BOOL CSceneScriptFileContext::getScriptFileName(char *dst)
{
    switch (this->type) {
        case SCRIPT_TYPE_EVENT:
            sprintf(dst, "eve%08d.ssd_", this->code);
            break;
        case SCRIPT_TYPE_MATCH:
            sprintf(dst, "mch%08d.ssd_", this->code);
            break;
        case SCRIPT_TYPE_ACTION:
            sprintf(dst, "act%06d.ssd_", this->code);
            break;
        case SCRIPT_TYPE_HELP:
            sprintf(dst, "help%06d.ssd_", this->code);
            break;
